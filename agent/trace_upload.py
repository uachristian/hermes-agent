...
* **Zero LLM turn.** Like NanoClaw's runner-handled command, this is a
  deterministic export — it never spends a model call. The CLI/gateway
  slash handlers and the ``hermes trace upload`` subcommand all call
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    except Exception:  # pragma: no cover - redactor must never break upload
        return text
